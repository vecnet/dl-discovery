module Blacklight::HierarchyHelper
  ...
  def render_qfacet_value(facet_solr_field, item, options ={})
    (link_to_unless(options[:suppress_link], item.value, add_facet_params(facet_solr_field, item.qvalue), :data=> {:'facet-name' => facet_solr_field, :'facet-solr-value' => item.qvalue}, :class=>"facet_select") + " " + render_facet_count(item.hits)).html_safe
  end
  ...
  def render_selected_qfacet_value(facet_solr_field, item)
    ...
    # Commenting out the removal X span because getting the correct url into the ajax setup of dl-discovery is a huge pain with this gem... apologies.
    # + " " +
